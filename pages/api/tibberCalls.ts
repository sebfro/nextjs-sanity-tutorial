...
export const temp = async () => {
	return graphql({ schema, source: '{ hello }', rootValue: root }).then(
		response => {
...
const consumption =
	'consumption(resolution: DAILY, last: 10) { nodes { from to cost unitPrice unitPriceVAT consumption consumptionUnit currency } }';
const owner = 'owner { firstName lastName contactInfo { email mobile }}';
const currentSubscription =
	'currentSubscription {priceInfo {current {total energy tax startsAt }}}';
